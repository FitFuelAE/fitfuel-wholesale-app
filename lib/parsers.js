 ...
 */
const NOT_A_NAME =
  /^(?:invoice|order|due)\s+date\b|^terms?\b|^p\.?\s*o\.?\s*#|^vat\s*no\b|^trn\b|^balance\s+due\b|^(?:bill|ship)\s+to\b|^aed\s*[\d,]|^[\d.,%\s-]*$/i;
...
export function billToName(lines) {
  ...
  if (sameRow && !NOT_A_NAME.test(sameRow)) return sameRow;
